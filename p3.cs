...
then a randomS object, then a oscillateS object and then repeats 
...
namespace p3
{
    class Driver
    {
        ...
        static void testObjects(iSeq [] arr)
        {   
        ...
        static void initialize(iSeq [] arr)
        {
            for (int i = 0; i < SIZE; i++)
            {
                arr[i] = new iSeq(rng(MIN_START,MAX_START), rng(MIN_P,MAX_P));
                
                if (i+1 < SIZE)
                {
                    arr[++i] = new randomS(rng(MIN_START,MAX_START), rng(MIN_P,MAX_P));
                }

                if (i+1 < SIZE)
                {
                    arr[++i] = new oscillateS(rng(MIN_START,MAX_START), rng(MIN_P,MAX_P));
                }
            }
        }
