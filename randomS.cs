...
on the outcome of a rng(CASE_TURNON, CASE_TURNOFF); where 
CASE_TURNON is a const int set to 0 which sets the object to 
active and CASE_TURNOFF is a const int set to 2 which sets the 
object to inactive. P value is derived from the parent class, 
iSeq. Zero is not a multiple of any number, so when zero is 
the first number in the sequence, we will return that number 
upon the first query(). I've created a rng() function to aid 
the arbitrary case switching of the states and for the random 
number addition to the sequence. I've also created a uint: 
firstNum that holds the first number in the sequence; I do this
because we need this number to be able to reset the function 
to the first value that was entered into the program 
(unsigned version).
*/
using System;
namespace p3
{
    public class randomS : iSeq
    {
        ...
        private const int CASE_TURNON = 0;
        private const int CASE_TURNOFF = 2;
        ...
        */
        ...
        {
            uint caseNum = Convert.ToUInt32(rng(CASE_TURNON, CASE_TURNOFF));

            if (caseNum == CASE_TURNON)
